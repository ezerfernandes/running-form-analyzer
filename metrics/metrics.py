...
class Metrics:
    def __init__(self, config: Config):
        ...
        self.key_metrics = {
            ...
            "left_shank_angle_assessment": "",
            "right_shank_angle_assessment": "",
    ...
    def calculate_metrics(
        self, valid_keypoints, timestamp: float
    ) -> Tuple[np.ndarray, Dict[str, Any]]:
        if self.start_time is None:
            self.start_time = timestamp

        metrics = {
            metric: default_value for metric, default_value in self.key_metrics.items()
        }
        ...
        metrics["recommendations"] = (
            self.recommendations_calculator.get_recommendations(metrics)
        )
