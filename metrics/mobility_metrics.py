...
class MobilityMetrics:
    ...
    def reset_metrics(self):
        ...
        for limb in self.limbs:
            ...
            setattr(
                self, f"{limb}_direction", 0
            )  # 0: neutral, 1: forward, -1: backward
    ...
    def process_swing(self, limb: str, current_angle: float):
        ...
        if current_angle > prev_angle:
            ...
            if direction == 1:  # Continuing forward swing
                setattr(
                    self,
                    f"{limb}_current_forward",
                    max(getattr(self, f"{limb}_current_forward"), current_angle),
                )
            ...
            if direction == -1:  # Continuing backward swing
                setattr(
                    self,
                    f"{limb}_current_backward",
                    max(getattr(self, f"{limb}_current_backward"), abs(current_angle)),
                )
    ...
    def finalize_swing(self, limb: str, swing_type: str):
        ...
        if "arm" in limb:
            assessment = (
                AssessmentCalculator.assess_forward_arm_swing(current_max)
                if swing_type == "forward"
                else AssessmentCalculator.assess_backward_arm_swing(current_max)
            )
        else:  # hip
            assessment = (
                AssessmentCalculator.assess_forward_hip_swing(current_max)
                if swing_type == "forward"
    ...
    def update_assessments(self, metrics: Dict[str, any]):
        for limb in self.limbs:
            ...
            metrics[f"max_{limb}_backward_swing"] = getattr(
                self, f"{limb}_max_backward"
            )
            metrics[f"{limb}_forward_swing_assessment"] = getattr(
                self, f"{limb}_forward_assessment"
            )
            metrics[f"{limb}_backward_swing_assessment"] = getattr(
                self, f"{limb}_backward_assessment"
            )
