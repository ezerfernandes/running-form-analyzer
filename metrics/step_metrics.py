...
class StepMetrics:
    def __init__(
        self,
        filter_type: Literal["temporal", "kalman", "none"],
        detection_axis: Literal["x", "y"],
    ):
        self.left_foot_strike_detector = FootStrikeDetector(
            filter_type=filter_type, detection_axis=detection_axis
        )
        self.right_foot_strike_detector = FootStrikeDetector(
            filter_type=filter_type, detection_axis=detection_axis
        )
        ...
        self.left_assessments = {"knee": "", "hip_ankle": "", "shank": ""}
        self.right_assessments = {"knee": "", "hip_ankle": "", "shank": ""}
    ...
    def update(
        self,
        valid_keypoints: Dict[int, np.ndarray],
        timestamp: float,
        metrics: Dict[str, any],
        angles: Dict[str, float],
    ):
        left_foot_strike, right_foot_strike = self.detect_foot_strikes(
            valid_keypoints, timestamp, metrics
        )
    ...
    def detect_foot_strikes(
        ...
        if 15 in valid_keypoints:  # Left ankle
            left_foot_strike, _, _ = self.left_foot_strike_detector.update(
                valid_keypoints[15], timestamp
            )
            if left_foot_strike:
                self.left_step_count += 1

        if 16 in valid_keypoints:  # Right ankle
            right_foot_strike, _, _ = self.right_foot_strike_detector.update(
                valid_keypoints[16], timestamp
            )
    ...
    def calculate_step_counts(self, metrics: Dict[str, any]):
        ...
        if metrics["elapsed_time"] > 0:
            metrics["steps_per_minute"] = (
                self.total_step_count / metrics["elapsed_time"]
            ) * 60
    ...
    def update_assessments(
        ...
        metrics: Dict[str, any],
        angles: Dict[str, float],
        left_foot_strike: bool,
        right_foot_strike: bool,
    ):
        if left_foot_strike:
            if "left_knee_angle" in angles:
                self.left_assessments["knee"] = AssessmentCalculator.assess_knee_angle(
                    angles["left_knee_angle"], is_front=True
                )
                self.left_knee_angle_at_strike = angles["left_knee_angle"]
            if "left_hip_ankle_angle" in angles:
                self.left_assessments["hip_ankle"] = (
                    AssessmentCalculator.assess_hip_ankle_angle(
                        angles["left_hip_ankle_angle"]
                    )
                )
                self.left_hip_ankle_angle_at_strike = angles["left_hip_ankle_angle"]
            if "left_shank_angle" in angles:
                self.left_assessments["shank"] = (
                    AssessmentCalculator.assess_shank_angle(angles["left_shank_angle"])
                )
                self.left_shank_angle_at_strike = angles["left_shank_angle"]

        if right_foot_strike:
            if "right_knee_angle" in angles:
                self.right_assessments["knee"] = AssessmentCalculator.assess_knee_angle(
                    angles["right_knee_angle"], is_front=True
                )
                self.right_knee_angle_at_strike = angles["right_knee_angle"]
            if "right_hip_ankle_angle" in angles:
                self.right_assessments["hip_ankle"] = (
                    AssessmentCalculator.assess_hip_ankle_angle(
                        angles["right_hip_ankle_angle"]
                    )
                )
                self.right_hip_ankle_angle_at_strike = angles["right_hip_ankle_angle"]
            if "right_shank_angle" in angles:
                self.right_assessments["shank"] = (
                    AssessmentCalculator.assess_shank_angle(angles["right_shank_angle"])
                )
        ...
        metrics["right_hip_ankle_angle_assessment"] = self.right_assessments[
            "hip_ankle"
        ]
        ...
        metrics["right_hip_ankle_angle_at_strike"] = (
            self.right_hip_ankle_angle_at_strike
        )
