...
class AngleMetrics:
    ...
    def calculate(
        ...
        self.angles = AngleCalculator.calculate_all_angles(
            valid_keypoints, self.angles, self.config
        )
    ...
    def update_static_angles_assessment(self, metrics: Dict[str, any]):
        if "head_angle" in self.angles:
            metrics["head_angle_assessment"] = AssessmentCalculator.assess_head_angle(
                metrics["head_angle"]
            )

        if "trunk_angle" in self.angles:
            metrics["trunk_angle_assessment"] = AssessmentCalculator.assess_torso_angle(
                abs(metrics["trunk_angle"])
            )

        if self.config.side == "left":
            if "left_elbow_angle" in self.angles:
                metrics["left_elbow_angle_assessment"] = (
                    AssessmentCalculator.assess_elbow_angle(metrics["left_elbow_angle"])
                )
        else:
            if "right_elbow_angle" in self.angles:
                metrics["right_elbow_angle_assessment"] = (
                    AssessmentCalculator.assess_elbow_angle(
