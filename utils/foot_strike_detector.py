...
class FootStrikeDetector:
    def __init__(
        self,
        filter_type: Literal["temporal", "kalman", "none"] = "kalman",
        window_size: int = 10,
        ...
        self.max_threshold = (
            0.1  # Maximum threshold to ensure detection in large movements
        )

    def update(
        self, ankle_position: np.ndarray, timestamp: float
    ) -> Tuple[bool, float, float]:
        position = (
            ankle_position[0] if self.detection_axis == "x" else ankle_position[1]
        )
        ...
        if self.detection_axis == "x":
            ...
            if (
                ...
                and self.positions[-2] < self.positions[-3]
                and self.positions[-3] - self.positions[-2] > self.threshold
            ):
            ...
            if (
                self.positions[-2] < self.positions[-1]
                and self.positions[-2] < self.positions[-3]
                and self.positions[-1] - self.positions[-2] > self.threshold
            ):
                foot_strike_detected = True
        ...
        if foot_strike_detected:
            if (
                ...
                foot_strike_detected = (
                    False  # Reset if it's too soon after the last detection
                )
    ...
    def _update_adaptive_threshold(self):
        ...
        self.threshold = max(
            min(self.threshold, self.max_threshold), self.min_threshold
        )
