...
class MetricsLogger:
    def __init__(self, log_dir: str = "tests/logs"):
        ...
        self.log_file = os.path.join(
            log_dir, f"run_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
        self.csv_file = None
    ...
    def log_metrics(self, timestamp: float, metrics: Dict[str, Any]):
        if not self.metrics:
            print(
                "Error: Metrics have not been initialized. Call initialize_logging() first."
            )
            return
    ...
    def rename_log_file(self):
        new_filename = input(
            "Enter new filename (or press Enter to keep current name): "
        )
        if new_filename:
    ...
    def delete_log_file(self):
        if self.log_file and os.path.exists(self.log_file):
            confirm = input("Do you want your log file to be deleted? (y/n): ")
            if confirm.lower() == "y":
