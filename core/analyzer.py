...
class Analyzer:
    ...
    def process_frame(self) -> Tuple[bool, np.ndarray]:
        ...
        if model_output is not None:
            ...
            keypoint_coords, keypoint_confs = extract_keypoints(
                model_output, frame.shape[0], frame.shape[1]
            )
            valid_keypoints = get_valid_keypoints(
                keypoint_coords, keypoint_confs, confidence_threshold=0.3
            )
        else:
        ...
        metrics, angles = self.metrics_calculator.calculate_metrics(
            valid_keypoints, current_time
        )
